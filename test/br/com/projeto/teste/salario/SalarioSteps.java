...
public class SalarioSteps {
	...
	@Dado("^O sistema de gestão de funcionários iniciará com os seguintes data$")
	...
	@Quando("^O chefe aumentar o salário do funcionário com o id '(\\d+)' por (\\d+)%$")
	public void o_chefe_aumentar_o_salario_do_funcionário_com_o_id_por(final int id, final int aumentoPorcentual) throws Throwable {
		gerenciadorDeSalario.aumentaSalario(id, aumentoPorcentual);
	}
	
	@Entao("^O salário para o empregado  com o id (\\d+)'' sera mostrado o valor de (\\d+)$")
